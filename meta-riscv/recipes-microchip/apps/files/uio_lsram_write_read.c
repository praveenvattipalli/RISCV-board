...
uint32_t memread[5]={0x12345678,0x98abcdef,0x05050505,0x34343434,0xAAAAAAAA};
  ...
  while(1){
	printf("\t # Enter a digit between 0 to 4 to write a pattern into lsram or\n");
	printf("\t # Enter 5 to read or\n");
	...
	printf("Writing LSRAMs with  the pattern 0x%x is in progress\n",memread[d]); 
	for(i=0;i<(MMAP_SIZE/4);i++){
		*(mem_ptr+i)=memread[d];

	}
	printf("writing to LSRAM completed\n");
  }
}
  // Should never reach
